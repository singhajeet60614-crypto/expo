...
| refresh_ad_hoc_provisioning_profile | boolean | Optional. When `true`, refreshes the managed ad hoc provisioning profile from App Store Connect before an iOS internal build starts so registered test devices are included. Same behavior as [`eas build --refresh-ad-hoc-provisioning-profile`](/eas/cli/#eas-build). Requires `platform: ios` and a profile with [`distribution: internal`](/eas/json/#distribution) and [`credentialsSource: remote`](/eas/json/#credentialssource). See [internal distribution on CI](/build/internal-distribution/#automation-on-ci-optional). |
