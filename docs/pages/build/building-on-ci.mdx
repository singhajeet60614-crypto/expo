...
For iOS internal distribution builds that must include newly registered test devices, set `refresh_ad_hoc_provisioning_profile: true` on the build job. See the [build job parameters](/eas/workflows/pre-packaged-jobs#build).
